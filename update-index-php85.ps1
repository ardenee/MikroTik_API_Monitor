...
'@
    ...
    if ($content -notmatch [regex]::Escape("`$_SESSION['router_user'] = `$routerUser;")) {
        $markerCrLf = "        header('Content-Type: application/json; charset=utf-8');`r`n        echo json_encode(["
        $replacementCrLf = "        `$_SESSION['router_user'] = `$routerUser;`r`n        `$_SESSION['router_pass'] = `$routerPass;`r`n`r`n        header('Content-Type: application/json; charset=utf-8');`r`n        echo json_encode(["
        if ($content.Contains($markerCrLf)) {
            ...
            $replacementLf = "        `$_SESSION['router_user'] = `$routerUser;`n        `$_SESSION['router_pass'] = `$routerPass;`n`n        header('Content-Type: application/json; charset=utf-8');`n        echo json_encode(["
...
Update-PhpFile $interfacePath {
    ...
    $sessionNeedle = "`$_SESSION['router_user']"
    if (-not $content.Contains($sessionNeedle)) {
